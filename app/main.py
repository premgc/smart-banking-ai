...
try:
    ...
    with st.expander('💸 Expense Breakdown'):
        ...
        st.dataframe(expense_df, use_container_width=True) if not expense_df.empty else st.info('No expense breakdown available.')
...
if query:
    ...
    try:
        ...
        if 'day' in q and 'upi' in q and 'deposit' in q:
            answer = format_single_column_daywise(get_upi_deposit_daily(), '📅 Day-wise UPI Deposit', 'Deposit', '💰')
        elif 'day' in q and 'upi' in q and ('withdraw' in q or 'debit' in q):
            answer = format_single_column_daywise(get_upi_withdrawal_daily(), '📅 Day-wise UPI Withdrawal', 'Withdrawal', '💸')
        elif 'day' in q:
            txn_type = detect_txn_type(q)
            if txn_type:
                answer = format_daywise_summary(get_transaction_daily_summary(txn_type), f'📅 Day-wise {txn_type} Summary')
            else:
                available = ', '.join(get_unique_transaction_types())
                answer = f'❌ Please specify a transaction type.\n\nAvailable types: {available}'
        ...
        elif 'transaction types' in q or 'transaction codes' in q or 'unique transaction types' in q:
            txn_types = get_unique_transaction_types()
            answer = '### 📋 Available Transaction Types\n\n' + '\n'.join([f'- {t}' for t in txn_types])
        else:
